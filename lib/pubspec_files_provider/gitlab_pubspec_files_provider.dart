...
class GitlabPubspecsProvider {
  ...
  Future<Iterable<String>> getPubspecFiles() async {
    final projectIds = await _getProjectIdsInGroup(groupId);
    final pubspecs = <String>[];

    for (final projectId in projectIds) {
      final pubspecPaths = await _getAllPubspecPaths(projectId);

      for (final path in pubspecPaths) {
        final content = await _retryWithDelay(
          () => _getPubspecContent(projectId, path),
          maxRetries,
  ...
  Future<List<String>> _getProjectIdsInGroup(String groupId) async {
    final url = Uri.parse('$gitlabApiUrl/groups/$groupId/projects');
    final headers = {'Private-Token': token};
    final response = await _retryWithDelay(
  ...
  Future<List<String>> _getAllPubspecPaths(String projectId) async {
  ...
  Future<String> _getPubspecContent(String projectId, String filePath) async {
    final url = Uri.parse(
